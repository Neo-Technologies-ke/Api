...
export class PersonController extends MembershipBaseController {
  ...
  public async bulkDelete(req: express.Request<{}, {}, { personIds?: string[]; ids?: string[] }>, res: express.Response): Promise<any> {
    return this.actionWrapper(req, res, async (au) => {
      if (!au.checkAccess(Permissions.people.edit)) return this.json({}, 401);
      else {
        const { personIds, ids } = req.body;
        const idsToDelete = personIds || ids;
        if (!idsToDelete || !Array.isArray(idsToDelete) || idsToDelete.length === 0) {
          return this.json({ error: "personIds array is required" }, 400);
        }
  ...
  public async queryMembers(req: express.Request<{}, {}, any>, res: express.Response): Promise<any> {
    return { message: "PersonController queryMembers reached", body: req.body };
  }
