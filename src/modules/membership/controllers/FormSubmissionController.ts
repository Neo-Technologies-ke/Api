      ...
      if (ids?.length > 0) {
        ...
        if ((people as any[])?.length > 0) {
          ...
          (people as any[]).forEach((p: Person) => {
            if (p.email) promises.push(EmailHelper.sendTemplatedEmail(Environment.supportEmail, p.email, church.name, Environment.b1AdminRoot, "New Submissions for " + form.name, contents, "ChurchEmailTemplate.html"));
          });
